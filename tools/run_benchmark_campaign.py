#!/usr/bin/env python3
"""Run the C++ core benchmark and preserve reproducibility metadata."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--random-instances", type=int)
    parser.add_argument("--families")
    parser.add_argument("--sparse-sizes")
    parser.add_argument("--dense-sizes")
...
def main() -> int:
    ...
    if not args.cli.exists():
        raise SystemExit(f"Benchmark executable does not exist: {args.cli}")
    ...
    optional_arguments = (
        ...
        ("--random-instances", args.random_instances),
        ("--families", args.families),
        ("--sparse-sizes", args.sparse_sizes),
        ("--dense-sizes", args.dense_sizes),
    )
    ...
    if args.plot:
        plot_script = Path(__file__).with_name("plot_benchmarks.py")
        subprocess.run(
            [sys.executable, str(plot_script), "--input", str(raw_csv), "--output-dir", str(run_dir / "figures")],
            check=True,
        )

    print(f"Benchmark campaign saved to: {run_dir}")
    return 0
